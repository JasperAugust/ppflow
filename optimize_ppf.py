...
def main():
    ...
    parser.add_argument('-t', '--tag', type=str, default='233k1410')
    ...
    for i in range(mark, len(dataset)):
        ...
        count = 0
        for batch in tqdm(loader, desc='Optimizing Peptides', dynamic_ncols=True):
            torch.set_grad_enabled(False)
            batch = recursive_to(batch, args.device)
            ...
            # Process the final optimization step
            final_step = traj_batch[config.sampling.num_steps]
            pos_atom_new_bb3 = manifold_to_euclid(
                final_step[0],
                final_step[1],
                final_step[2],
                batch['pos_heavyatom'][:, :, :4],
            ...
            aa_new = final_step[3].cpu()  # Amino acid residues
            pos_atom_new_bb3 = pos_atom_new_bb3.cpu()
            mask_atom_new = batch['mask_gen_pos'][:,:,None].repeat(1,1,4).cpu()
            
            for i in range(aa_new.size(0)):
                print("Batch keys:", batch.keys())
                print("Batch mask_gen_pos shape:", batch['mask_gen_pos'].shape)
                peptide_patch_idx = torch.where(batch['mask_gen_pos'][i])[0].cpu()
                data_tmpl = _index_select_data(batch[i], peptide_patch_idx)
                aa = aa_new[i][peptide_patch_idx]
                mask_ha = mask_atom_new[i][peptide_patch_idx]
                pos_ha = pos_atom_new_bb3[i][peptide_patch_idx]
                pos_ha_translate = pos_ha + batch['pos_center_org'][i]

                save_path = os.path.join(config.output_dir, f'optimized_{count:04d}.pdb')
                save_pdb({
                    'chain_nb': data_tmpl['chain_nb'],
                    'chain_id': data_tmpl['chain_id'],
                    'resseq': data_tmpl['resseq'],
